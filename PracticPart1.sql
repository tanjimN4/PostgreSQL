...
SELECT department_name,count(department_name) as count_salary FROM employees
JOIN departments USING(department_id)
GROUP BY department_name
...
SELECT extract(year from hire_date) as year,count(extract(YEAR FROM hire_date)) FROM employees
GROUP BY YEAR
